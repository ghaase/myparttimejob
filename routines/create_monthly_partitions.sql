...
comment 'takes table_name and schema_name and creates partitions for 3 months forward'
begin
    ...
    declare     c_partitions        cursor for
        ...
        where
            ...
            pr.day >= curdate() and pr.day < date_add(curdate(), interval 3 month) and
            not exists
                (
                select
                    1
                ...
                where
                    isp.partition_name = concat(pt.partition_base_name,pr.month_partition_suffix) and
