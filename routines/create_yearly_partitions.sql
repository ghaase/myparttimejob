drop procedure if exists myparttimejob.create_yearly_partitions$$

create procedure myparttimejob.create_yearly_partitions
(
    in  l_table_name    varchar(64),
    in  l_table_schema  varchar(64)
)
comment 'takes table_name and schema_name and creates partitions for 2 years forward'
begin
    ...
    declare     c_partitions        cursor for
        select
            concat(pt.partition_base_name,pr.year_partition_suffix) as partition_name,
            pr.year_partition_filter as partition_filter
        from
        ...
        where
            ...
            pr.day >= curdate() and pr.day < date_add(curdate(), interval 2 year) and
            not exists
                (
                select
                    1
                ...
                where
                    isp.partition_name = concat(pt.partition_base_name,pr.year_partition_suffix) and
